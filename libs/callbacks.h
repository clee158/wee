...
/*
 *	CLARIFICATION: copy_constructor_type, destructor_type, default_constructor type
 *
 *		- The declarations below are just declaring function pointers that will be used
 *		with the names above, but they are just acting as "shells" for functions that have
 *		the same parameter & return type
 *
 *		- You can pass in same parameter & return type functions to it
 *			
 *			= why this file has so many constructor, destructor, and default constructors implemented,
 *			so the user can choose & define which type of constructor they want to pass in according
 *			to the type of elements they are using
 *	
 *
 *	WHY?:
 *
 *		= This is because C does not automatically support destructors and copy constructors
 *			for classes!
 *
 *			--> you need to explicitly store constructor and destructor functions in your
 *			vector so it knows how to handle its elements!
 */
